...
# run viper
tf_activities <- run_viper(cpm_gex, dorothea_regulon,
  options =  list(method = "scale", minsize = 4,
  eset.filter = FALSE, cores = 1,
  verbose = FALSE))

## Which TFs to display? The ones with larger average between the replicates.
...
TFs_to_consider_max <- 
  ...
  dplyr::group_by(TF, patient) %>% 
  dplyr::mutate(average = mean(activity)) %>% 
  dplyr::ungroup() %>% dplyr::select(TF, patient,average) %>% 
  dplyr::distinct() %>% group_by(patient) %>% 
  dplyr::slice_max(abs(average), n=5) %>% dplyr::pull(TF) %>% unique()
...
TFs_to_consider_sd <- 
  ...
  dplyr::mutate(average = mean(activity)) %>% 
  dplyr::ungroup() %>% dplyr::select(TF, patient,average) %>% 
  dplyr::distinct() %>% group_by(TF) %>% 
  dplyr::mutate(sd_TF = sd(average)) %>% dplyr::ungroup() %>%
  dplyr::distinct(TF, sd_TF) %>%
  dplyr::slice_max(sd_TF, n=20) %>% dplyr::pull(TF) %>% unique()
```

### Heatmap considering the TFs with more activity

```{r, warning=FALSE, message=FALSE, dpi=300}
dorothea_matrix <- t(as.matrix(tf_activities[TFs_to_consider_max,]))
dorothea_matrix <- dorothea_matrix[sort(rownames(dorothea_matrix)), ]


pheatmap(t(dorothea_matrix) ,treeheight_row=0,treeheight_col=0, cluster_cols =  FALSE)
```


### Heatmap considering the TFs with more variablity

```{r, warning=FALSE, message=FALSE, dpi=300}
dorothea_matrix <- t(as.matrix(tf_activities[TFs_to_consider_sd,]))
dorothea_matrix <- dorothea_matrix[sort(rownames(dorothea_matrix)), ]
